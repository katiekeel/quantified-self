...
function getMealFoodsAfterSubmit(foods) {
  $(".food-meal-add-buttons").find('form').submit(function(event) {
    ...
    // should grab meal name also
    let mealId = ($(this).attr("id").split("-")[1])
    foods = getFoodInfo();
    postAjax(mealId, foods)
  })
}
...
function postAjax(mealId, foods) {
  foods.forEach(function(food) {
    $.ajax({
      method: 'POST',
      url: "http://localhost:3000/api/v1/meals/" + mealId + "/foods/" + food.id,
    }).done(function(data) {
      HTMLHelper.addNewMealFoodsToTable(mealId, foods);
